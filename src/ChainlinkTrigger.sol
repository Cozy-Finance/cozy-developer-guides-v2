...
/**
 * @notice A trigger contract that takes two addresses: a truth oracle and a tracking oracle. This trigger ensures the
 * two oracles always stay within the given price tolerance; the delta in prices can be equal to but not greater than
 * the price tolerance.
 */
contract ChainlinkTrigger is BaseTrigger {
  ...
  /// @param _priceTolerance The maximum percent delta between oracle prices that is allowed, expressed as a wad.
  ...
  function programmaticCheck() internal view returns (bool) {
    ...
    // We round up when calculating the delta percentage to accommodate for precision loss to ensure that the state
    // becomes triggered when the delta is greater than the price tolerance. When the delta is less than or exactly
    // equal to the price tolerance, the resulting rounded up value will not be greater than the price tolerance, as expected.
